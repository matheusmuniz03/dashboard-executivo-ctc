...
function getDeploymentBaseUrl() {
  const rawUrl = process.env.SEED_DATA_BASE_URL || process.env.VERCEL_URL || "";
  if (!rawUrl) {
    return "";
  }
...
async function readPublicJsonFile(relativePath) {
  const baseUrl = getDeploymentBaseUrl();
  if (!baseUrl || typeof fetch !== "function") {
    return null;
  }

  const url = new URL(relativePath.replace(/\\/g, "/"), `${baseUrl.replace(/\/+$/, "")}/`);
  const response = await fetch(url, { cache: "no-store" });
  if (response.status === 404) {
    return null;
  }
  if (!response.ok) {
    throw new Error(`Falha ao ler ${relativePath} no deployment: HTTP ${response.status}`);
  }
...
async function readJsonFile(relativePath) {
  ...
  const publicValue = await readPublicJsonFile(relativePath);
  if (publicValue !== null) {
    return publicValue;
  }

  throw lastError || new Error(`Arquivo obrigatÃ³rio nÃ£o encontrado: ${relativePath}`);
}
